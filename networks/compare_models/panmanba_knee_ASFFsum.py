...
class Net(nn.Module):
    ...
    def forward(self,ms,pan):
        ...
        # ms_f = self.spa_cat_fusion(torch.cat([ms_f,pan_f],dim=1))
        # ms_f = self.spa_sum_fusion(torch.add(ms_f,pan_f))
        ...
        # ms_f = self.final_fusion0(torch.cat([ms_f,ms_fre],dim=1))
        ...
        # residual_fre = 0
        # f_fre1, residual_fre = self.deep_fusion5(f_fre, residual_fre, ms_f)
        # f_fre1 = f_fre1 + f_fre
        # ms_f1,residual_ms_f = self.deep_fusion6(ms_f,residual_ms_f,f_fre)
        # ms_f1 = ms_f1 + ms_f
        # f_fre2, residual_fre = self.deep_fusion7(f_fre1, residual_fre, ms_f1)
        # f_fre2 = f_fre2 + f_fre1
        # ms_f2,residual_ms_f = self.deep_fusion8(ms_f1,residual_ms_f,f_fre1)
        # ms_f2 = ms_f2 + ms_f1
        # # ms_f, residual_ms_f = self.deep_fusion7(ms_f1, residual_ms_f, f_fre1)
        # # ms_f, residual_ms_f = self.deep_fusion8(ms_f, residual_ms_f, f_fre1)
        # ms_f2 = self.patchunembe(ms_f2,(h,w))       
        # f_fre2 = self.patchunembe(f_fre2,(h,w))
        # ms_f = self.final_fusion0(torch.cat([ms_f2,f_fre2],dim=1))
        
        out = self.cnn_fusion0(torch.cat([ms_f,ms_fre],dim=1))
        out = self.cnn_fusion1(out)
        out1, out2 = torch.chunk(out, 2, dim=1)
        [out1, out2] = self.exchange([out1, out2])
        ms_f = self.cnn_fusion2(torch.cat([out1, out2], dim=1)) + ms_f
