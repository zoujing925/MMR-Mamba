...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x,x_size):
        ...
        x = x.reshape(B, self.nc, x_size[0], x_size[1])  # B Ph*Pw C
        return x
...
class VisionMamba(nn.Module):
    def __init__(self, 
        ...
        # channel = self.embed_dim//patch_size**2
        channel = 48
        self.patchunembe = PatchUnEmbed(channel)
    ...
    def forward(self, T2, T1, return_features=False, inference_params=None, if_random_cls_token_position=False, if_random_token_rank=False):
        ...
        if return_features:
            return x
        x = self.patchunembe(x,(224, 224))
        # print("after patchunembe: ", x.shape)
        # x = self.head(x)
