...
class DCM(nn.Module):
    ...
    def forward(self, x, convoluted):  # x for cal kernel, input is convoluted
        ...
        # print('generated_filter:', generated_filter.shape) # [4, 32, 3, 3]
        convoluted = self.c1(convoluted)
        b, c, h, w = x.shape
        # print('convoluted:', convoluted.shape, 'x:', x.shape)
        # [1, b * c, h, w], c = self.channels
        convoluted = convoluted.reshape(1, b * c, h, w)
        # print('convoluted:', convoluted.shape)  # [1, 128, 240, 240]
        # [b * c, 1, filter_size, filter_size]
        generated_filter = generated_filter.reshape(b * c, 1, self.filter_size, self.filter_size)
        # print('generated_filter:', generated_filter.shape)      # [128, 1, 3, 3]
        ...
        # print('output:', output.shape)      # [4, 32, 240, 240]
...
class Fusion_dynamic(nn.Module):
    def __init__(self, n_feat, kernel_size=3, padding=1, filter_ks=3, bias=False):
        ...
        self.conv1x1_fusion = nn.Conv2d(n_feat*2, n_feat, kernel_size=3, padding = padding, bias=bias)
        
        self.fusion = nn.Sequential(nn.Conv2d(n_feat,n_feat,3,1,1),
