...
from .mca import CrossTransformerEncoderLayer
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.norm_cro1= LayerNorm1(dim, LayerNorm_type)
        self.norm_cro2 = LayerNorm1(dim, LayerNorm_type)
    ...
    def forward(self, ms,pan):
        # print('ms in TransformerBlock',ms.shape)
        ms_norm = self.norm_cro1(ms)
        # print('ms_norm',ms_norm.shape)
        pan_norm = self.norm_cro2(pan)
        ms = ms+self.cro(ms_norm,pan_norm)
...
class PatchUnEmbed(nn.Module):
    ...
    def forward(self, x,x_size):
        B,HW,C = x.shape
        x = x.transpose(1, 2).reshape(B, self.nc, x_size[0], x_size[1])  # B Ph*Pw C
        return x
...
class Net(nn.Module):
    def __init__(self,num_channels=None,base_filter=None,args=None):
        ...
        self.stride2=8
        self.patch_size2=8
        self.embed_dim2 = base_filter*self.stride2*self.patch_size2
        ...
        ######################### change patch size and stride
        self.ms_to_token2 = PatchEmbed(in_chans=self.base_filter,embed_dim=self.embed_dim2,patch_size=self.patch_size2,stride=self.stride2)
        self.pan_to_token2 = PatchEmbed(in_chans=self.base_filter,embed_dim=self.embed_dim2,patch_size=self.patch_size2,stride=self.stride2)
        self.ms_fre_to_token = PatchEmbed(in_chans=self.base_filter,embed_dim=self.embed_dim2,patch_size=self.patch_size2,stride=self.stride2)
        ...
        self.num_heads = 16
        self.dim_atten = 16
        self.atten_embed1 = nn.Conv2d(base_filter,self.num_heads,3,1,1)
        self.atten_embed2 = nn.Conv2d(base_filter,self.num_heads,3,1,1)
        self.cross_attention1 = TransformerBlock(self.dim_atten, num_heads=self.num_heads, ffn_expansion_factor=4, bias=True, LayerNorm_type='BiasFree')
        self.cross_attention2 = TransformerBlock(self.dim_atten, num_heads=self.num_heads, ffn_expansion_factor=4, bias=True, LayerNorm_type='BiasFree')
        self.cross_attention3 = TransformerBlock(self.dim_atten, num_heads=self.num_heads, ffn_expansion_factor=4, bias=True, LayerNorm_type='BiasFree')
        self.cross_attention4 = TransformerBlock(self.dim_atten, num_heads=self.num_heads, ffn_expansion_factor=4, bias=True, LayerNorm_type='BiasFree')
        
        self.cross_transformer1 = CrossTransformerEncoderLayer(self.embed_dim2, num_heads=8)
        self.cross_transformer2 = CrossTransformerEncoderLayer(self.embed_dim2, num_heads=8)
        self.cross_transformer3 = CrossTransformerEncoderLayer(self.embed_dim2, num_heads=8)
        self.cross_transformer4 = CrossTransformerEncoderLayer(self.embed_dim2, num_heads=8)
        ...
        self.output = Refine(self.dim_atten,1)       
        
        self.final_fusion1 = nn.Sequential(nn.Conv2d(self.dim_atten,self.dim_atten,3,1,1),nn.LeakyReLU(0.1,inplace=True))
    ...
    def forward(self,ms,pan):
        ...
        ms_f = self.atten_embed1(ms_f)
        pan_f = self.atten_embed2(pan_f)
        ...
        # ms_f = self.final_fusion(torch.cat([ms_f,ms_fre],dim=1))
        
        # f_spa = self.cross_transformer1(ms_f,pan_f)
        # f_spa = self.cross_transformer2(f_spa,pan_f)
        # f_spa = self.cross_transformer3(f_spa,pan_f)
        # f_spa = self.cross_transformer4(f_spa,pan_f)
        
        f_spa = self.cross_attention1(ms_f,pan_f)
        f_spa = self.cross_attention2(f_spa,pan_f)
        f_spa = self.cross_attention3(f_spa,pan_f)
        f_spa = self.cross_attention4(f_spa,pan_f)
        
        # print('after cross_attention', 'f_spa', f_spa.shape)
        ...
        # ms_f = self.patchunembe(f_spa,(h,w))
        ms_f = f_spa
        ms_f = self.final_fusion1(ms_f) + ms_f
